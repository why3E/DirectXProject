...
void CGameFramework::BuildObjects()
{
	...
	CDuckPlayer* pAirplanePlayer = new CDuckPlayer(m_pd3dDevice,m_pd3dCommandList, m_pScene->GetGraphicsRootSignature());
...
void CGameFramework::BuildObjects_change()
{
	...
	CDuckPlayer* pAirplanePlayer = new CDuckPlayer(m_pd3dDevice,
	...
	wParam, LPARAM lParam)
{
	...
	case WM_KEYUP:
		...
		case VK_CONTROL:
			...
			break;
		case VK_SPACE:
			if (m_pPlayer && m_pScene->type != 0) m_pPlayer->Jump();
			break;
		default:
...
void CGameFramework::ProcessInput()
{
   ...
   뒤(로컬 z-축)로 이동한다. ‘Page Up’과 ‘Page Down’ 키를 누르면 플레이어를 위/아래(로컬 y-축)로 이동한다.*/
	...
	{
		if (pKeyBuffer[0x57] & 0xF0) dwDirection |= DIR_FORWARD;  // W 키
		if (pKeyBuffer[0x53] & 0xF0) dwDirection |= DIR_BACKWARD; // S 키
		if (pKeyBuffer[0x41] & 0xF0) dwDirection |= DIR_LEFT;     // A 키
   ...
   레이어를 x-축 또는 y-축으로 회전한다.*/
	...
	if ((dwDirection != 0) || (cxDelta != 0.0f) || (cyDelta != 0.0f))
	{
			...
			if (dwDirection) m_pPlayer->Move(dwDirection, 200.0f * m_GameTimer.GetTimeElapsed(),
				true);
